...
void gpuRenderEngine::bindObject(const object& obj){
   ...
   glBufferData(GL_ARRAY_BUFFER, mod.getVertices().size() * sizeof(GLfloat), mod.getVertices().data(), GL_STATIC_DRAW);
...
void gpuRenderEngine::render(){
   ...
   vec4 bgColor = getColor(Color::Black);
   ...
   for (const gpuObject& gpuObject : m_objects){
      ...
      vec4 color = getColor(objRef.color());
