...
// a lot of lines when you are changing a lot of states, saving previous states and resetting those states.
// this also increases the likelyhood of state leaks. using RAII wrappers for each state change ensures that
// state changes are only persistant for the scope they are created in.
